...
class ProjectBuilder:
    ...
    def _encrypt_python_files(self) -> Dict[str, Any]:
        ...
        for file_info in python_files:
            if not self._should_exclude_from_encryption(file_info):
                ...
                self.logger.info(f"排除加密: {file_info['file_path']} (相对路径: {file_info['relative_path']})")
        ...
        for file_info in filtered_files:
            ...
            if build_file_path.exists():
                # 创建加密文件路径
                encrypted_path = build_file_path.with_suffix(build_file_path.suffix + BuildConstants.PYTHON_ENCRYPTED_EXT)
                
                # 加密文件
                self.crypto.encrypt_file(str(build_file_path), str(encrypted_path), encryption_key)
    ...
    def _encrypt_onnx_files(self) -> Dict[str, Any]:
        ...
        for file_info in filtered_files:
            ...
            if build_file_path.exists():
                # 创建加密文件路径
                encrypted_path = build_file_path.with_suffix(build_file_path.suffix + BuildConstants.ONNX_ENCRYPTED_EXT)
